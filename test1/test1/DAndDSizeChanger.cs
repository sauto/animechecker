...
namespace test.DAndDSizeChanger
{
    /// <summary>
        ...
        void mouseListner_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Left)
            {
                ...
                if ((status & DAndDArea.Top) == DAndDArea.Top)
                {
                    ...
                    //縮めたときはTopが上がるのを再現 差分をなくすようにしている？移動量が減る？
                    //e.Yが「イベントが発生したコントロール上の位置」を表すのでTopを移動させればコントロールの位置も変わる
                    //Topを変更せず、移動量が変わらないまま高さだけが増減していくと高さが0または65535になってしまう
                    sizeChangeCtrl.Top += h - sizeChangeCtrl.Height;//結果的に移動量を足している
                }
